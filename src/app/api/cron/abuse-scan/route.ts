...
const PAYLOAD_API_URL = process.env.NEXT_PUBLIC_STOREFRONT_URL ? process.env.PAYLOAD_API_URL : process.env.PAYLOAD_API_URL;
...
export async function GET(request: Request): Promise<Response> {
	...
	for (const [agent_id, list] of byAgent.entries()) {
		console.warn(
			`[abuse] agent=${agent_id} flags=${list.length} rules=${list.map((f) => f.rule).join(",")}`,
		);
...
async function fetchRecentActivity(): Promise<AgentActivityEntry[]> {
	...
	const url = `${PAYLOAD_API_URL}/agent-activity?where[created_at][greater_than]=${encodeURIComponent(since)}&limit=1000&sort=-created_at`;
	try {
...
async function suspendAgent(agentId: string, evidence: AbuseFlag[]): Promise<boolean> {
	try {
		const res = await fetch(`${PAYLOAD_API_URL}/agents?where[id][equals]=${encodeURIComponent(agentId)}`, {
			...
			body: JSON.stringify({
				status: "suspended",
				notes: `Auto-suspended ${new Date().toISOString()} — ${evidence.length} abuse flags: ${evidence.map((e) => e.rule).join(", ")}`,
			}),
		});
		if (!res.ok) {
