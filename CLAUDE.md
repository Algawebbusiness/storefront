...
- 🚧 **F9 (next)** — Phase F closure. End-to-end smoke matrix run-through (per F8 spec-pinning doc — MCP Inspector current/old + curl + Claude Desktop). Hard bundle budget enforcement v build script (>250 KB gz → exit 1, CI fail). Telemetry hooks (count `view.error` events + handshake-timeout fallbacks) for adoption tracking. Final spec re-check proti aktuální `apps.mdx` draft revs.
