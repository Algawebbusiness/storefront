...
> - **Fáze F (F1–F5): 🚧 IN PROGRESS** — MCP Apps infrastructure + security + catalog + detail views:
...
>   - **F6–F9 čekají** — cart (paired-tool first time), checkout/order (paired), fallback, docs.
...
- 🚧 **F6 (next)** — Cart preview view + first paired-tool. `get_cart` (model-visible: cart-state + public lines) + `get_cart_full` (`visibility:["app"]`, includes `buyer.*` + `shipping_address.*` PII). Iframe used `bridge.fetchAppData("get_cart", {cart_id})` for the full payload. Plus standalone app-only tools: `update_cart_line` + similar (registerAppTool with `visibility:["app"]`, hidden from `tools/list`). UiMessage `cart.proceed_to_checkout` finally consumed.
- F7–F9 — checkout/order (paired), fallback, docs.
