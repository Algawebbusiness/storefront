...
> - **Fáze F (F1–F6): 🚧 IN PROGRESS** — MCP Apps infrastructure + security + catalog + detail + cart views (first paired-tool):
...
- 🚧 **F7 (next)** — Checkout summary + Order receipt views. `update_checkout` migrace → registerAppTool s `_meta.ui.resourceUri = ui://saleor/checkout-summary.html`. Paired `get_checkout_summary` + `_full` (address + buyer paired tool podle vzoru F6). `complete_checkout` migrace → registerAppTool s `_meta.ui.resourceUri = ui://saleor/order-receipt.html`; nový OrderReceipt komponent. UiMessage `checkout.confirm_requested` + `checkout.payment_failed` se konečně použijí.
- F8–F9 — fallback strategy + error boundaries + spec-version resilience, docs/tests/telemetry.
