...
> - **Fáze F (F1–F7): 🚧 IN PROGRESS** — full shopping flow: catalog → detail → cart → review → pay → receipt, with paired-tool PII isolation throughout:
...
>   - **F8–F9 čekají** — fallback strategy + error boundaries, docs/tests/telemetry.
...
- 🚧 **F8 (next)** — Fallback strategy + error boundaries + spec-version resilience. Iframe load timeout → graceful JSON dump fallback (host bez MCP Apps podpory). React `ErrorBoundary` na entry úrovni s `sendUiMessage({kind:"view.error", view, code})`. Hard bundle budget enforced v build script (>250 KB gz → CI fail). Env-injected `window.__STOREFRONT_URL__` pro absolutní View-order URLs (vyřeší F7 placeholder). Spec version negotiation z `ui/initialize` handshake.
- F9 — docs/tests/telemetry; final phase F closure.
